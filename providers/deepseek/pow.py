...
class DeepSeekHash:
    ...
    def init(self: 'DeepSeekHash', wasm_path: str) -> None:
    ...
    def _write_to_memory(self: 'DeepSeekHash', text: str) -> tuple[int, int]:
    ...
    def calculate_hash(self: 'DeepSeekHash', algorithm: str, challenge: str, salt: str, difficulty: int, expire_at: int) -> float:
...
class DeepSeekPoW:
    def __init__(self: 'DeepSeekPoW') -> None:
        self.hasher = DeepSeekHash().init(WASM_PATH)
    
    def solve_challenge(self: 'DeepSeekPoW', config: Dict[str, Any]) -> str:
